...
def nonlin_cg(comm, experiment, args, postprocess=None):
    ...
    if postprocess and comm.rank == 0:
        postprocess.log("dict", "static",{"SID": args.sim_id, "MODEL":args.mod, "MASS LUMPING" : args.mass_lumping, "EXPERIMENT":args.exp, "mesh res.": args.dh, "dt":args.dt, "dim":args.dim, "T": args.T, "ksp": args.ksp_type_n, "pc" : args.pc_type_n, "seed":args.seed, "cs":args.cs, "approx_method": args.lam})
    ...
    a, L = variational_form(dxL, d1, q1, d0, q0, b, c, dt, args, H=H, noise = noise)
    ...
    if postprocess:
        ...
        postprocess.log_functions(0.0, {"d":d_out, "q":q_out, "n": noise}, mesh = domain, meshtags = meshtags)
    ...
    while t < args.T:
        ...
        if postprocess: 
            d_out, q_out = u0.sub(0).collapse(), u0.sub(1).collapse()
            postprocess.log_functions(t, {"d":d_out, "q":q_out, "n": noise}, mesh = domain, meshtags = meshtags)
...
def variational_form(dxL, d1, q1, d0, q0, b, c, dt, args, H=None, noise = None):
        """
