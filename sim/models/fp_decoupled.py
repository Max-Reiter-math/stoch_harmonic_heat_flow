...
def fp_decoupled(comm, experiment, args, postprocess=None):
    ...
    a2, a3, L2, L3 = variational_forms(dxL, d1, q1, dl0, ql0, d0, q0, b_test, c_test, dt, args, H=H, noise=noise)
    #!SECTION
...
def variational_forms(dxL, d1, q1, dl0, ql0, d0, q0, b, c, dt, args, H=None, noise = None):
        """
        ...
        if args.gamma != 0.0:
            a_11 -= 0.5*dt* args.gamma *inner(ql0, a_times_b_times_c(dl0_,d1,c))*dxL
            L_1  += 0.5*dt* args.gamma *inner(ql0, a_times_b_times_c(dl0_,d0,c))*dxL

        if not( noise is None ):
            if args.dim == 2:
                d0_orth = as_vector([-d0[1], d0[0]])
                d1_orth = as_vector([-d1[1], d1[0]])
                L_1 += 0.5*args.cs*inner(noise,dl0_)*inner(d0_orth,c)*dxL
                a_11-= 0.5*args.cs*inner(noise,dl0_)*inner(d1_orth,c)*dxL
            else:
                L_1 += 0.5*args.cs*inner(cross(d0, noise),c)*dxL
                a_11-= 0.5*args.cs*inner(cross(d1, noise),c)*dxL
        ...
        L_2 -= 0.5 * q_elastic_energy(args, dl0, dl0, d0, b, H = H) 
        ...
        """
        return form(a_11), form(a_22), form(L_1), form(L_2)
